...
public class RopeRoot : MonoBehaviour
{
    ...
    private const int IGNORE_SPAGHETTI_LAYER = 11;
    private const bool USE_COLLIDERS = false;   // Too much going on, no noodle physics
    ...
    void Awake() {
        if (RigidBodyContainer == null) {
            ...
            RigidBodyContainer.layer = IGNORE_SPAGHETTI_LAYER;
        }
    ...
    private void AddChildren(Transform parent) {
        for (int i = 0; i < parent.childCount; i++) {
            ...
            representative.layer = IGNORE_SPAGHETTI_LAYER;
            //rigidbody
